...
In the Four Pillars of OOP chapter, we introduced the four pillars of Object-Oriented Programming—Encapsulation, Inheritance, Polymorphism, and Abstraction—and saw how each one solves a different design problem as software grows.

In this chapter, we explore the first pillar, Encapsulation, and learn how objects protect their own data by controlling how it is accessed and modified.
...
1. **Public vs. private.** Create a `Product` class with a public `name` and a private `__price`. Add a `get_price()` method to read it. Confirm that `product.__price` raises an `AttributeError` from outside the class, while `product.get_price()` works.

2. **Setter with validation.** Add a `set_price(new_price)` method to `Product` that rejects any value less than or equal to `0`, printing a message instead of updating `__price`. Test it with both a valid and an invalid price.

3. **Bring back the bank account.** Build a `BankAccount` class with a private `__balance`, set through `__init__`. Add:
   - `deposit(amount)` — only accepts positive amounts.
   - `withdraw(amount)` — only allows withdrawing up to the current balance.
   - `get_balance()` — returns the current balance.

   Try withdrawing more than the balance holds, and confirm the object refuses the change instead of allowing it.
...
-   [W3Schools — Python Classes and Objects (Private Attributes & Methods)](https://www.w3schools.com/python/python_classes.asp)
