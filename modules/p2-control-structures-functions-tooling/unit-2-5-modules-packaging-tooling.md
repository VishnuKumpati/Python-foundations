...
- **Describe** what Pytest does in a professional Python project, and why teams rely on it.
...
- **Keep the dependency file under version control** — commit `requirements.txt` alongside your code, so teammates can rebuild the exact same setup.
...
**Stage 3 — Prove the module works with a Pytest test**
...
- **UPI / Payment Systems:** Payment gateway code depends on cryptography and networking packages installed via `pip`; a virtual environment (or its production equivalent, a container) ensures the exact same versions run in testing and in the live system handling real money.
